...
def initial_setup(config: Dict) -> Dict[str, xr.DataArray]:
    try:
        ...
        for product_id, product_dir in sorted_product_paths:
            csv_file = os.path.join(product_dir, f'{config["DATE"]}_uncertainty_metrics.csv')
...
def add_revenue_objective(model: cp.Model, revenues_xr: xr.DataArray, price_indices: cp.IntVar) -> None:
...
def add_constraints(model: cp.Model, xr_data: Dict[str, xr.DataArray], price_indices: cp.IntVar) -> None:
...
def add_price_constraints(model: cp.Model, xr_data: Dict[str, xr.DataArray], price_indices: cp.IntVar, 
                          products: xr.DataArray, lower_percent: int, upper_percent: int) -> None:
...
def solve_model(model: cp.Model, price_indices: cp.IntVar, xr_data: Dict[str, xr.DataArray], config: Dict) -> pd.DataFrame:
    try:
        if model.solve():
            solution = pd.DataFrame(columns=['Product','Catalan Credit A','Catalan Cash A','National Credit A',
                                             'National Cash A','National DE','Min Credit A','Max Credit A','Demand','Revenue'])
            ...
            for p in range(xr_data['prices_xr'].shape[0]):
                ...
                logger.info(f" Product {product}: Best Price Pair Index {price_index}, Prices: {best_price_pair}, Revenue: {revenue}, Demand: {demand}")
